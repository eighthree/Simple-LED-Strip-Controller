(function($){
  ...
  /*
  // Load WS2812FX Modes in localstorage
  */
  function getFXModes() {
    $.ajax({
        url: "modes.json",
        dataType: "json",
        success: function (e) {
          localStorage.setItem('modesList', JSON.stringify(e));
          alert("Light Modes Stored. Reloading page...");
          setTimeout(function(){location.reload();}, 5000);
        }
    });
  }

  if (localStorage.getItem('modesList') != null) {
    var items = [];
    var FXmodes = localStorage.getItem('modesList');
    var FXmodes = $.parseJSON(FXmodes);
    
    $.each( FXmodes.effects, function( key, val ) {
